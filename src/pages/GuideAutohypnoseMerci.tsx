...
/* ─────────────────────────────────────────────
   Thank You page — guide "L'Autohypnose au Quotidien"
...
const GuideAutohypnoseMerci: React.FC = () => {
  ...
  useEffect(() => {
    if (!location.state) { navigate('/guide-autohypnose', { replace: true }); }
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href="https://novahypnose.fr/guide-autohypnose-merci" />
      </Helmet>

      <div className="cz">
        <svg width="0" height="0" style={{ position: 'absolute' }} aria-hidden="true">
          <defs>
            <filter id="riso-full-am">
              <feTurbulence type="turbulence" baseFrequency="0.018 0.022" numOctaves={2} seed={4} result="turb" />
            ...
            <filter id="paperGrain-am" x="0" y="0" width="100%" height="100%">
        ...
        <section style={{ position: 'relative', overflow: 'hidden', padding: '100px 0 80px', minHeight: '60vh', display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
            <svg width="100%" height="100%" viewBox="0 0 1440 600" preserveAspectRatio="xMidYMid slice">
              <g filter="url(#riso-full-am)">
                <path d="M 0 0 L 400 0 C 500 60, 520 140, 480 220 C 440 300, 320 340, 200 320 C 80 300, 0 220, 0 140 Z" fill="#F2A12E" opacity="0.6" />
              </g>
              <g filter="url(#riso-full-am)" style={{ mixBlendMode: 'multiply' }}>
                <path d="M 1440 600 L 1000 600 C 920 540, 880 460, 900 380 C 920 300, 1020 260, 1160 280 C 1300 300, 1440 380, 1440 460 Z" fill="#2B4BA0" opacity="0.55" />
              </g>
              <rect width="1440" height="600" filter="url(#paperGrain-am)" opacity=".15" />
            </svg>
          </div>
                  ...
                  >
                    <div style={{ aspectRatio: '16/9', overflow: 'hidden', background: 'rgba(43,75,160,.08)' }}>
                      <img src={article.image_url || article.storage_image_url || '/placeholder.svg'} alt={article.title}
                        style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.3s' }} loading="lazy" />
                    </div>
