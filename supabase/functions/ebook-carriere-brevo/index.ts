...
serve(async (req) => {
  ...
  if (!BREVO_API_KEY) {
    console.error('ebook-carriere-brevo — BREVO_API_KEY manquante dans les variables d\'environnement')
    return new Response(
  ...
  try {
    ...
    console.log('ebook-carriere-brevo — Body recu:', body)
    ...
    // l'ajouter directement a la liste "ebook-carriere" via listIds, en un seul appel.
    ...
    console.error('ebook-carriere-brevo — Erreur:', error)
    return new Response(
