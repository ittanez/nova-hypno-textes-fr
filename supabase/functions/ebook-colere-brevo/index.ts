...
serve(async (req) => {
  ...
  if (!BREVO_API_KEY) {
    console.error('ebook-colere-brevo — BREVO_API_KEY manquante dans les variables d\'environnement')
    return new Response(
  ...
  try {
    ...
    console.log('ebook-colere-brevo — Body recu:', body)
    ...
    // l'ajouter directement a la liste 20 (ebook-colere) via listIds, en un seul appel.
    ...
    console.error('ebook-colere-brevo — Erreur:', error)
    return new Response(
